...
export default function ReservationCard({
  ...
  useEffect(() => {
    ...
    fetch(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/appointments/${room._id}/appointments`,
      {
        method: "GET",
        headers: {
          authorization: `Bearer ${session?.user.token}`,
      ...
      .then((res) => res.json())
      .then((data) => {
        const datata: any = data.data.appointments
          .filter((appointment: any) => {
            return dayjs(new Date(appointment.date)).isSame(
              reservationTime,
              "day"
            );
          })
      ...
      if (session?.user.role === "user") {
        if (data && data.length <= 2) {
          ...
          if (
            ...
            startHour < cowokingStartHour ||
            (startHour == cowokingStartHour &&
              startMinute < cowokingStartMinute)
          ) {
            Swal.fire({
              title: "Reservation Failed",
            ...
            endHour > cowokingEndHour ||
            (endHour == cowokingEndHour && endMinute > cowokingEndMinute)
          ) {
            Swal.fire({
              title: "Reservation Failed",
            ...
            if (reservationData) {
              for (let i = 0; i < reservationData.length; i++) {
                ...
                if (
                  dayjs(timeToDate(time1.format("HH:mm"))).isBetween(
                    start,
                    end
                  ) ||
                  dayjs(timeToDate(time2.format("HH:mm"))).isBetween(
                    start,
                    end
                  ) ||
        ...
        if (
          ...
          startHour < cowokingStartHour ||
          (startHour == cowokingStartHour && startMinute < cowokingStartMinute)
        ) {
          Swal.fire({
            title: "Reservation Failed",
            text: "Start time must be after coworking open time",
      ...
      <div className=" flex flex-col w-full space-y-3">
        ...
        <div className=" flex flex-row space-y-3 w-full">
          <DateReserve
            disable={false}
            value={null}
            onChangeDate={(value: Dayjs) => setDate(value)}
          />
        </div>
      </div>
      <div className=" flex flex-row w-full space-x-5">
        <div className="flex flex-col space-y-3 w-1/2 ">
          ...
          <TimeReserve
            disable={false}
            value={null}
            onChangeTime={(value: Dayjs) => setTime1(value)}
          />
        </div>
        <div className="flex flex-col space-y-3 w-1/2 ">
          ...
          <TimeReserve
            disable={false}
            value={null}
            onChangeTime={(value: Dayjs) => setTime2(value)}
          />
        </div>
      </div>
      <div className=" flex flex-col w-full space-y-3">
        ...
        <div className=" flex flex-row space-y-3 w-full">
          <textarea
            ...
            className=" w-full h-40 border-2 p-3 rounded-md border-gray-300"
          />
        </div>
      </div>
      <button
